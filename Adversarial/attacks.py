...
class PGD_L2(Attacker):
    ...
    def attack(self, inputs: torch.Tensor, labels: torch.Tensor,
               noise: torch.Tensor = None, num_noise_vectors=1, targeted: bool = False, no_grad=False) -> torch.Tensor:
        if num_noise_vectors == 1:
            return self._attack(inputs, labels, noise, targeted)
        else:
            if no_grad:
                with torch.no_grad():
                    return self._attack_mutlinoise_no_grad(inputs, labels, noise, num_noise_vectors, targeted)
            else:
                    return self._attack_mutlinoise(inputs, labels, noise, num_noise_vectors, targeted)


    def _attack(self, inputs: torch.Tensor, labels: torch.Tensor,
    ...
    def _attack_mutlinoise(self, inputs: torch.Tensor, labels: torch.Tensor,
    ...
    def _attack_mutlinoise_no_grad(self, inputs: torch.Tensor, labels: torch.Tensor,
               noise: torch.Tensor = None, num_noise_vectors: int = 1,targeted: bool = False) -> torch.Tensor:
        """
        Performs the attack of the model for the inputs and labels.
